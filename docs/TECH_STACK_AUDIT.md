...
| **Canonical / sitemap** | `src/lib/seo/canonical.ts`, `src/app/sitemap/`, `sitemap-index.ts` |
| **RSS** | `/blog/rss` route |
...
|------|----------|---------|
| **Content audit** | `content_audit/` | E-E-A-T & content integrity (GoogleQualityAuditor, lazy-writing) |
| **Dependencies** | `content_audit/requirements.txt` | nltk, textblob, vaderSentiment, spacy, beautifulsoup4 |
...
- **Content audit (Python):** Pin versions in `content_audit/requirements.txt` (e.g. `nltk==3.8.x`) and note Python version (e.g. 3.10+) in content_audit/README.
